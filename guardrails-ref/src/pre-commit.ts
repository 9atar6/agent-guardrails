...
export function runSetupPreCommit(projectPath: string = ".", remove = false, dryRun = false): PreCommitResult {
  ...
  if (remove) {
    ...
    if (dryRun) {
      ...
      const wouldHusky = existsSync(huskyPath) && readFileSync(huskyPath, "utf-8").includes(GUARDRAILS_CHECK_MARKER);
      const wouldPreCommit = existsSync(preCommitPath) && readFileSync(preCommitPath, "utf-8").includes(GUARDRAILS_CHECK_MARKER);
      return {
